...
function createVideoBackground(source: string): HTMLVideoElement | null {
...
export function applyMiddleArea(): Dispose {
  const video = createVideoBackground(themeConfig.middleArea.video.source)
  const disposeBinding = waitForElement<HTMLElement>(
    '[data-slot="conversation"] > [data-phase], ._47VQSW_root',
    (root) => {
